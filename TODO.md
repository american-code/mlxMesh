...
- **Treasury balance monitoring** — Add `oim_treasury_balance` gauge (updated on every treasury credit/debit), alert threshold based on average daily coordination spend, and `oim_coordination_reward_skipped_total{reason="treasury_insufficient"}` counter when floor check fails. Current floor check prevents overdraft but provides no signal when rewards stop paying.
...
- **Power-of-two-choices for fast-lane dispatch** — Greedy "best measured TPS" routing creates herding — everyone gets sent to whichever node looks best, overloading it. Power-of-d-choices: sample two random candidate nodes and route to whichever has lighter current load. Provably avoids herding failure mode, few lines of change.
