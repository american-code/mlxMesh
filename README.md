...
| **Third-party security review** of the crypto + settlement paths | Wallet auth, attestation, and ledger debits are trust-critical | **Done** — external review completed. Prior to the external review, an internal multi-angle pass (crypto/TLS trust boundary, secrets/auth, DoS/resource exhaustion) found and fixed several real issues: identity-file permissions not tightened on rewrite; node payload-fetch SSRF guard only validated the initial URL (closed by `httpmw.SafeFetchClient` — per-redirect re-validation + connect-time IP check in the dialer against DNS-rebinding); federation-key bearer check used a non-constant-time `==` (now `crypto/subtle.ConstantTimeCompare`). A reviewer-facing scope package exists at [SECURITY.md](SECURITY.md) — assets, the five trust boundaries, the full cryptographic inventory, a threat→mitigation map, and a vulnerability-disclosure policy |
